...
def Test_Datasets(dataset_path,size, test_folder = 1):
    ...
    for file in data_list:
        #file = str(int(file))
        #print("\nStart Evaluate: ", file)
        pre = normalization(load(os.path.join(dataset_path,f'image_p0/{file}_P0.nii.gz'))).astype(np.float32)
        pos = normalization(load(os.path.join(dataset_path,f'image_p1/{file}_P1.nii.gz'))).astype(np.float32)
        sub = normalization(pos - pre)
        print(sub.shape)
        gt = load(os.path.join(dataset_path,f'label/{file}_GT.nii.gz')).astype(np.int16)
...
def Test_all_Datasets(dataset_path,size, test_folder = 1):
    ...
    for file in data_list:
        ...
        breast_mask =load(os.path.join(dataset_path, f'breastmask/{file}.nii.gz')).astype(np.int16)
