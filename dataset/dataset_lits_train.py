...
class Lits_DataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pre = self.normalization(self.load(os.path.join(self.root,f'image_p0/{file}_P0.nii.gz'))).astype(np.float32)
        pos = self.normalization(self.load(os.path.join(self.root,f'image_p1/{file}_P1.nii.gz'))).astype(np.float32)
        sub = pos - pre
        gt = self.load(os.path.join(self.root,f'label/{file}_GT.nii.gz')).astype(np.float32)
